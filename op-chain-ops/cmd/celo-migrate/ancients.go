...
func migrateAncientsDb(ctx context.Context, oldDBPath, newDBPath string, batchSize, bufferSize uint64) (numAncientsNewBefore uint64, numAncientsNewAfter uint64, lastAncient *RLPBlockElement, err error) {
	...
	if err != nil {
		return 0, 0, nil, fmt.Errorf("failed to open old freezer: %w", err)
	}
	defer func() {
	...
	if err != nil {
		return 0, 0, nil, fmt.Errorf("failed to open new freezer: %w", err)
	}
	defer func() {
	...
	if err != nil {
		return 0, 0, nil, fmt.Errorf("failed to get number of ancients in old freezer: %w", err)
	}

	numAncientsNewBefore, err = newFreezer.Ancients()
	if err != nil {
		return 0, 0, nil, fmt.Errorf("failed to get number of ancients in new freezer: %w", err)
	}
	...
	if numAncientsNewBefore >= numAncientsOld {
		...
		lastAncient, err = loadLastAncient(newFreezer, numAncientsNewBefore)
		if err != nil {
			return 0, 0, nil, err
		}
		return numAncientsNewBefore, numAncientsNewBefore, lastAncient, nil
	}
	...
	if err = g.Wait(); err != nil {
		return 0, 0, nil, fmt.Errorf("failed to migrate ancients: %w", err)
	}
	...
	if numAncientsNewAfter != numAncientsOld {
		return 0, 0, nil, fmt.Errorf("failed to migrate all ancients from old to new db. Expected %d, got %d", numAncientsOld, numAncientsNewAfter)
	}

	lastAncient, err = loadLastAncient(newFreezer, numAncientsNewAfter)
	if err != nil {
		return 0, 0, nil, err
	}
	...
	return numAncientsNewBefore, numAncientsNewAfter, lastAncient, nil
}
...
func loadLastAncient(freezer *rawdb.Freezer, numAncients uint64) (*RLPBlockElement, error) {
